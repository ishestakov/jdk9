...
if [ $JAVA_VERSION -eq "8" ]; then
	docker run -it --rm -v $SCRIPTPATH/lib/:/example/lib -v $SCRIPTPATH/mod/:/example/mod -v $SCRIPTPATH/target/java$JAVA_VERSION:/example/target openjdk:$JAVA_VERSION java -cp '/example/lib/*:/example/mod/*:/example/target/classes:/example/target/resources' com.github.ishestakov.coe.Main
else

...
function compile {
	...
	docker run -it --rm -v $SCRIPTPATH:/example -v $SCRIPTPATH/target/:/example/target/ -v $HOME:$HOME --user `id -u $USER`:`id -g $USER` -e HOME=$HOME  openjdk:$JAVA_VERSION /bin/bash /example/javac$JAVA_VERSION.sh
}
